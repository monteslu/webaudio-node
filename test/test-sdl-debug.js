...
try {
    ...
    const interval = setInterval(() => {
        elapsed += 100;
        if (elapsed % 1000 === 0) {
            console.log(`${elapsed/1000}s - Callbacks so far: ${callbackCount}, queued: ${audioDevice.queued}, playing: ${audioDevice.playing}`);
        }
        if (elapsed >= 3000) {
            clearInterval(interval);
            audioDevice.close();
            console.log(`\n✅ Done! Total callbacks: ${callbackCount}`);
            console.log(callbackCount > 0 ? '✅ SDL WORKS' : '❌ SDL BROKEN - callbacks never fired');
            process.exit(0);
        }
    }, 100);
