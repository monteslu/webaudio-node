...
async function testFormat(name, path, decoderFunc) {
    ...
    try {
        ...
        console.log(`  First 5 samples: [${Array.from(result.audioData.slice(0, 5)).map(v => v.toFixed(4)).join(', ')}]`);
...
async function main() {
    ...
    const results = {
        mp3: await testFormat('MP3', `${basePath}.mp3`, WasmAudioDecoders.decodeMP3.bind(WasmAudioDecoders)),
        wav: await testFormat('WAV', `${basePath}.wav`, WasmAudioDecoders.decodeWAV.bind(WasmAudioDecoders)),
        flac: await testFormat('FLAC', `${basePath}.flac`, WasmAudioDecoders.decodeFLAC.bind(WasmAudioDecoders)),
        ogg: await testFormat('OGG/Vorbis', `${basePath}.ogg`, WasmAudioDecoders.decodeVorbis.bind(WasmAudioDecoders)),
        aac: await testFormat('AAC', `${basePath}.aac`, WasmAudioDecoders.decodeAAC.bind(WasmAudioDecoders))
    };

    // Test auto-detection
    console.log('\n\nAuto-Detection Test:');
    console.log('='.repeat(50));
    for (const [format, path] of [['MP3', `${basePath}.mp3`], ['WAV', `${basePath}.wav`], ['FLAC', `${basePath}.flac`], ['OGG', `${basePath}.ogg`], ['AAC', `${basePath}.aac`]]) {
        const data = await readFile(path);
        const result = await WasmAudioDecoders.decode(data.buffer);
        console.log(`  ${format}: ✓ Auto-detected and decoded (${result.channels}ch, ${result.sampleRate}Hz)`);
    }
