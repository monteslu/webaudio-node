...
for (const bench of allBenchmarks) {
	...
	implementations.forEach((impl, i) => {
		...
		if (time !== null && time !== undefined) {
			...
			console.log(`  ${symbol} ${impl.color}${impl.name}${reset}: ${time.toFixed(2)}ms ${slowdown > 1.0 ? `(${slowdown.toFixed(2)}x slower)` : ''}`);
		} else {
...
implementations.forEach(impl => {
	...
	console.log(`${impl.color}${impl.name}${reset}: ${winCounts[impl.name]}/${allBenchmarks.length} wins (${pct}%)`);
});
