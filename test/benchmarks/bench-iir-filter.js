...
export async function benchmarkIIRFilter(OfflineAudioContext, iterations = 10) {
	...
	for (let i = 0; i < iterations; i++) {
		...
		const feedback1 = [1.0000, -3.0544, 3.8291, -2.2925, 0.5507];

		// Butterworth highpass at 200Hz
		const feedforward2 = [0.9149, -3.6597, 5.4895, -3.6597, 0.9149];
		const feedback2 = [1.0000, -3.8730, 5.6520, -3.6706, 0.8916];

		// Notch filter at 500Hz
		const feedforward3 = [0.9780, -1.8458, 2.6578, -1.8458, 0.9780];
		const feedback3 = [1.0000, -1.8359, 2.6481, -1.8261, 0.9561];

		// Peaking EQ at 2kHz
		const feedforward4 = [1.0623, -1.9318, 2.8013, -1.9318, 1.0623];
		const feedback4 = [1.0000, -1.9423, 2.8223, -1.9212, 0.9413];
